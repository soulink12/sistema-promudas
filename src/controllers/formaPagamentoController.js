...
const criarForma = async (req, res) => {
    try {
        const { nome, pagamento_posterior, conta_posterior } = req.body;
        if (!nome || nome.trim() === '') {
            return res.status(400).json({ erro: 'O campo nome é obrigatório.' });
        }
        const nova = await formaPagamentoService.criarForma(nome.trim(), pagamento_posterior ?? false, conta_posterior ?? false);
...
const atualizarForma = async (req, res) => {
    try {
        ...
        const { nome, pagamento_posterior, conta_posterior, ativo } = req.body;
        const dados = {};
